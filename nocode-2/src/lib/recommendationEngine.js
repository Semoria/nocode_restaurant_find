...
export async function searchPoiStores(lng, lat) {
  try {
    ...
    let response = await fetch(
      `https://restapi.amap.com/v3/place/around?key=${AMAP_KEY}&location=${lng},${lat}&radius=2000&keywords=奶茶|咖啡|饮品&output=JSON`
    );
    ...
    if (!data.pois || data.pois.length === 0) {
      response = await fetch(
        `https://restapi.amap.com/v3/place/around?key=${AMAP_KEY}&location=${lng},${lat}&radius=4000&keywords=奶茶|咖啡|饮品&output=JSON`
      );
